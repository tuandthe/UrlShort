...
import { LOGIN_FORM_TEXT } from "@/features/auth/constants/authUi.constants";

export default function LoginForm() {
  const { form, isLoading, loginWithGoogle, onSubmit } = useLoginForm();
  ...
  return (
    <div className="space-y-8">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="ml-1 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{LOGIN_FORM_TEXT.email}</FormLabel>
                <FormControl>
                  <div className="relative">
                    <Mail className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input className="pl-10" placeholder={LOGIN_FORM_TEXT.emailPlaceholder} type="email" {...field} />
                  </div>
                </FormControl>
                <FormMessage />
              </FormItem>
            ...
            render={({ field }) => (
              <FormItem>
                <div className="mb-2 ml-1 flex items-center justify-between">
                  <FormLabel className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{LOGIN_FORM_TEXT.password}</FormLabel>
                  <a className="text-[11px] font-bold uppercase tracking-[0.14em] text-primary hover:underline" href="#">
                ...
                <FormControl>
                  <div className="relative">
                    ...
                    <Input className="pl-10" placeholder={LOGIN_FORM_TEXT.passwordPlaceholder} type="password" {...field} />
                  </div>
                </FormControl>
                <FormMessage />
              </FormItem>
          ...
          <Button type="submit" className="h-12 w-full text-xs font-bold uppercase tracking-[0.16em]" disabled={isLoading}>
            {isLoading ? LOGIN_FORM_TEXT.signingIn : LOGIN_FORM_TEXT.signIn}
      ...
      >
        ...
        {LOGIN_FORM_TEXT.continueWithGoogle}
      </Button>
    </div>
  );
}
